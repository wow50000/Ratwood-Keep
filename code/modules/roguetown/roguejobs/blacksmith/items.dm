...
/*
AP's cream, brush and glint below.
Just about a 1:1.
*/
...
/obj/item/polishing_cream/attack_obj(obj/O, mob/living/user)
	...
	if(!thing.anvilrepair)
		return ..()
	if((HAS_TRAIT(user, TRAIT_SQUIRE) || user.mind.get_skill_level(thing.anvilrepair)) && thing.polished == 0 && obj_integrity <= max_integrity)
...
/obj/item/armor_brush
	icon = 'icons/roguetown/items/misc.dmi'
	name = "fine brush"
	desc = "A coarse brush for scrubbing armor thoroughly. Made of the finest Lupian hair."
	icon_state = "brush_0"
	w_class = WEIGHT_CLASS_SMALL
	smeltresult = null
	dropshrink = 0.8
...
/obj/item/armor_brush/attack_obj(obj/O, mob/living/user)
	...
	if(thing.polished == 1 && roughness)
		if((HAS_TRAIT(user, TRAIT_SQUIRE) || user.mind.get_skill_level(thing.anvilrepair)))
